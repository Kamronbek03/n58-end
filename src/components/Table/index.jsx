...
function Table({ data, fetchData }) {
  ...
  function handleNavigate(elID, el, isWatchedIcon) {
    if (isWatchedIcon) {
      const isExist = storedData.some((item) => item.id === elID);
      if (isExist) {
        const updatedData = storedData.filter((item) => item.id !== elID);
                  ...
                  .map((product, index) => {
                    ...
                    return (
                      <TableRow
                        key={index}
                        onClick={() => {
                          handleNavigate(product.id, product, false);
                        }}
                      >
                        <TableData>
                          <TableInfo>
                        ...
                        <td>
                          <TablePercents>
                            <img
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleNavigate(product.id, product, true);
                              }}
                            />
                            <p>
                              {percents > 0 ? (
          ...
          <Footer>
            <Stack spacing={2} style={{ color: "#87CEEB" }}>
              <Pagination
                ...
                renderItem={(item) => (
                  <PaginationS
                    style={{ color: "#87CEEB", fontFamily: "Montserrat" }}
                    {...item}
                  />
                )}
              />
